...
from clients.parser import get_web_driver, get_web_drivers_pool, BaseParser
...
class SberSuperMarketProvider(Provider):
    ...
    async def get_product(self, product_id: ProductID) -> ProductEntity:
        ...
        """
        return await retry_by_exception(
    ...
    def _get_product(self, product_id: ProductID) -> ProductEntity:
        ...
        if product.is_empty:
            raise ProviderError('Product data is empty')
        return product
    ...
    def _get_product_price(self, product_id: ProductID) -> str:
        ...
        for _ in range(self.RETRY_COUNT):
            if price_data:= self._get_elements_data(self.product_price_path):
                return price_data[0].text.replace(' ', '').replace('₽', '')  # TODO: move to serializer
            self._get_product_page(product_id)
        return ''
    ...
    def _get_product_specifications(self, product_id: ProductID) -> list[dict[str, str]]:
        """
        ...
        for _ in range(self.RETRY_COUNT):
            if not (specs_names := self._get_elements_data(self.product_specs_names_path)):
                continue
            if not (specs_values := self._get_elements_data(self.product_specs_values_path)):
                continue
            names = [key.text for key in specs_names]
            values = [key.text for key in specs_values]
            specs_data = [
                {'name': key, 'value': value} for key, value
                in itertools.zip_longest(names, values, fillvalue='')
                if key
            ]
            return specs_data
        return []
    ...
    def _get_product_categories(self, product_id: ProductID) -> list[str]:
        ...
        for _ in range(self.RETRY_COUNT):
            if categories_data := self._get_elements_data(self.product_categories_path):
                categories = [category.text for category in categories_data if category.text]
                return categories
            self._get_product_page(product_id)
        return []
